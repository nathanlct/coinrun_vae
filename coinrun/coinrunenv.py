...
def init_args_and_threads(cpu_count=4,
    ...
    if Config.RESET_RANDSEED > -1:
        Config.SET_SEED = Config.RESET_RANDSEED
    if Config.RESET_NLEV > -1:
        Config.NUM_LEVELS = Config.RESET_NLEV
    int_args = np.array([int(is_high_difficulty), Config.NUM_LEVELS, int(Config.PAINT_VEL_INFO), Config.USE_DATA_AUGMENTATION, game_versions[Config.GAME_TYPE], Config.SET_SEED, rand_seed, Config.BG_INDEX, Config.TEST_BG_INDEX, Config.LARGE_RANDOM_FLAG, Config.YG_LEVEL, Config.AGENT_INDEX, Config.GROUND_INDEX]).astype(np.int32)
