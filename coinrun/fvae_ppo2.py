...
from VAE.factor_vae import FactorConvVAE
...
class Model(object):
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
                nsteps, ent_coef, vf_coef, max_grad_norm, z_size):
        sess = tf.get_default_session()

        train_model = policy(sess, z_size, ac_space, nbatch_train, nsteps)
        norm_update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        act_model = policy(sess, z_size, ac_space, nbatch_act, 1)
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, nsteps, gamma, lam, vae):
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            ...
            encoder_in = self.obs.astype(np.float32) / 255.0
            batch_z = self.vae.encode(encoder_in)
            # batch_z = self.vae.encode_mu_logvar(encoder_in)[0]
            actions, values, self.states, neglogpacs = self.model.step(batch_z, self.states, self.dones)
            mb_obs.append(batch_z.copy())
        ...
        encoder_in = self.obs.astype(np.float32) / 255.0
        batch_z = self.vae.encode(encoder_in)
        last_values = self.model.value(batch_z, self.states, self.dones)
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
            ...
            save_interval=0, load_path=None, vae_path=None, z_size=64):
    ...
    model = Model(policy=policy, ob_space=ob_space, ac_space=ac_space, nbatch_act=nenvs, nbatch_train=nbatch_train,
                    ...
                    max_grad_norm=max_grad_norm, z_size=z_size)
    
    use_coord_conv = (Config.USE_COORD_CONV != -1)
    vae = FactorConvVAE(z_size=z_size, batch_size=nenvs, is_training=False, reuse=False, gpu_mode=True, use_coord_conv=use_coord_conv)
    vae.load_json_full(vae_path)
    
    utils.load_all_params(sess)

    runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, lam=lam, vae=vae)
    ...
    can_save = True
    checkpoints = list(range(0,2049,2))
    #checkpoints = list(range(0,200,10))
    ...
    for update in range(1, nupdates+1):
        ...
        if can_save:
            ...
            for j, checkpoint in enumerate(checkpoints):
                if (not saved_key_checkpoints[j]) and (step >= (checkpoint * 1e5)):
                    saved_key_checkpoints[j] = True
                    save_model(str(checkpoint) + '000K')
