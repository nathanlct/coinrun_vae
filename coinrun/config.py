...
class ConfigSingle(object):
    ...
    def __init__(self):
        ...
        type_keys.append(('lstm', 'use_lstm', int, 8425, True))
        ...
        type_keys.append(('lr', 'learning_rate', float, 25e-4))
        ...
        # Additional arguments for experiments
        km_keys.append(('bg_index', 'bg_index', int, 0)) # index of background
        km_keys.append(('test_bg_index', 'test_bg_index', int, 0))
        km_keys.append(('target_update_freq', 'target_update_freq', int, 5)) # update frequency of target net
        km_keys.append(('reset_flag', 'reset_flag', int, 1e6)) # manually reset the evaluation net
        km_keys.append(('eps_decay', 'eps_decay', float, 0.995))
        km_keys.append(('greedy_eps', 'greedy_eps', float, 0.2))
        km_keys.append(('fm_coeff', 'fm_coeff', float, 0.0))
        km_keys.append(('real_thres', 'real_thres', float, 0.9))
        km_keys.append(('reset_lstm', 'reset_lstm', int, -1))
        km_keys.append(('large_random_flag', 'large_random_flag', int, 0))
        km_keys.append(('meta_batch', 'meta_batch', int, 10))
        km_keys.append(('meta_window', 'meta_window', int, 5))
        km_keys.append(('meta_nlstm', 'meta_nlstm', int, 128))
        km_keys.append(('meta_lr', 'meta_lr', float, 0.1))
        km_keys.append(('meta_test_flag', 'meta_test_flag', int, 0))
        km_keys.append(('mc_iter', 'mc_iter', int, 5))
        km_keys.append(('reset_randseed', 'reset_randseed', int, -1))
        km_keys.append(('reset_nlev', 'reset_nlev', int, -1))
        km_keys.append(('vae_path', 'vae_path', str, ''))
        km_keys.append(('vae_z_size', 'vae_z_size', int, 64))
        km_keys.append(('mask_path', 'mask_path', str, ''))
        km_keys.append(('mask_val', 'mask_val', float, 0.0))
        km_keys.append(('test_log_name', 'test_log_name', str, 'test_log'))
        km_keys.append(('test_num_eval', 'test_num_eval', int, -1))

        yg_keys.append(('yg_level', 'yg_level', int, -1))
        yg_keys.append(('extract_seed', 'extract_seed', int, -1))
        yg_keys.append(('extract_rank', 'extract_rank', int, -1))
        yg_keys.append(('use_coord_conv', 'use_coord_conv', int, -1))
        yg_keys.append(('ground_index', 'ground_index', int, 0)) # index of ground
        yg_keys.append(('agent_index', 'agent_index', int, 0)) # index of agent
        self.RES_KEYS = []
        ...
        self.km_keys = km_keys
        self.yg_keys = yg_keys
    ...
    def initialize_args(self, use_cmd_line_args=True, **kwargs):
        ...
        for km in self.km_keys:
            parser.add_argument('-' + km[0], '--' + self.deprocess_field(km[1]), type=km[2], default=km[3])
        
        for yg in self.yg_keys:
            parser.add_argument('-' + yg[0], '--' + self.deprocess_field(yg[1]), type=yg[2], default=yg[3])
