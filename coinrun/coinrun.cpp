...
// additional parameters by km
int KM_BG_INDEX = 0;
int KM_TEST_BG_INDEX = 0;
int KM_LARGE_RANDOM_TRAIN_FLAG = 0;

// additional parameters by younggyo
int YG_LEVEL = -1;
int YG_GROUND_INDEX = 0;
int YG_AGENT_INDEX = 0;
...
void state_reset(const std::shared_ptr<State>& state, int game_type)
{
  ...
  if (USE_LEVEL_SET) {
    if (YG_LEVEL==-1) {
      level_index = global_rand_gen.randint(0, NUM_LEVELS);
    } else {
      level_index = YG_LEVEL;
    }
  ...
  if (KM_LARGE_RANDOM_TRAIN_FLAG==0) { // Small-scale experiments
    // KM: manually change the background 
    if (KM_BG_INDEX > 0 && KM_BG_INDEX <= int(bg_images.size())) {
      state->bg_n = global_rand_gen.randint(0, KM_BG_INDEX);
    } else {
      state->bg_n = maze_gen.randn(bg_images.size());
    }

    // YG: manually change the agent
    if (YG_AGENT_INDEX > 0 && YG_AGENT_INDEX <= int(player_themesl.size())) {
      agent.theme_n = global_rand_gen.randint(0, YG_AGENT_INDEX);
    } else {
      agent.theme_n = maze_gen.randn(player_themesl.size());
    }

    // YG: manually change the ground
    if (YG_GROUND_INDEX > 0 && YG_GROUND_INDEX <= int(ground_themes.size())) {
      state->ground_n = global_rand_gen.randint(0, YG_GROUND_INDEX);
    } else {
      state->ground_n = maze_gen.randn(ground_themes.size());
    }
  
    // KM: change the background for test
    if (KM_TEST_BG_INDEX > 0 && KM_TEST_BG_INDEX <= int(bg_images.size())) {
      state->bg_n = KM_TEST_BG_INDEX-1;
      if (KM_TEST_BG_INDEX == 30){
        state->bg_n = global_rand_gen.randint(1, bg_images.size());
      }
    }
  } else if (KM_LARGE_RANDOM_TRAIN_FLAG==1) { // Large-scale training case
  ...
  } else if (KM_LARGE_RANDOM_TRAIN_FLAG==2) { // Large-scale test case
...
void initialize_args(int *int_args) {
  ...
  KM_BG_INDEX = int_args[7];
  KM_TEST_BG_INDEX = int_args[8];
  KM_LARGE_RANDOM_TRAIN_FLAG = int_args[9];
  YG_LEVEL = int_args[10];
  YG_AGENT_INDEX = int_args[11];
  YG_GROUND_INDEX = int_args[12];
